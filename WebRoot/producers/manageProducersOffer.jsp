...
function addProducerOffers(){
	popUpWithServerData("/CTDomestic/addProducerOfferInput.action", "Manage Offers",600,800);
}
function updateProducerOffers()
{alert($("#offerId").val());
	popUpWithServerData("/CTDomestic/updateProducerOfferInput.action?raTraceOffers.offerId="+$("#offerId").val(), "Manage Offers",600,800);
}
</script>
<form name="offerForm" action="producersOffer">
	<h2>My Offers</h2>
	<jmesa:tableModel id="tag" items="${listOffer}" maxRows="8"
		...
		<jmesa:htmlTable width="100%">
			<jmesa:htmlRow>
				<jmesa:htmlColumn property="offerId" title="Select">
			...
			<input type="button" onclick="addProducerOffers()" value="Add" id="button_style"/>
			<input type="button" onclick="updateProducerOffers()" value="Update" id="button_style"/>
			<s:submit value="Delete" action="deActiveProducerOffers" theme="simple" align="center" id="button_style"></s:submit>
		...
		<script type="text/javascript">
            function onInvokeAction(id) {alert("12"+id);
             ...
             function onInvokeExportAction(id) {alert("2"+id);
                ...
                location.href = '/CTDomestic/producersOffer?' + parameterString;
            }
