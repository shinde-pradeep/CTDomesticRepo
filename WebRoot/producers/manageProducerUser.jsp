...
function addTraders(){
	popUpWithServerData("/CTDomestic/addProducerInput.action", "Manage Producer",500,800);
}
function updateTraders()
{alert($("#userId").val());
	popUpWithServerData("/CTDomestic/updateProducerInput.action?raUserMaster.userId="+$("#userId").val(), "Manage Producer",500,800);
}
</script>
<form name="userForm" action="producersUser">
	...
	<jmesa:tableModel id="tag" items="${traderList}" maxRows="8"
		...
		<jmesa:htmlTable width="100%">
			<jmesa:htmlRow>
				<jmesa:htmlColumn property="userId" title="Select">
			...
			<s:submit value="Delete" action="deActiveTrader" theme="simple" align="center" id="button_style"></s:submit>
		...
		<script type="text/javascript">
            function onInvokeAction(id) {alert("12"+id);
             ...
             function onInvokeExportAction(id) {alert("2"+id);
                ...
                location.href = '/CTDomestic/producersUser?' + parameterString;
            }
