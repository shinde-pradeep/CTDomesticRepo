...
window.parent.location.href='tradersOffer';
}

$(document).ready(function() {
				$("#frm_add_traderOffer").validationEngine()
				});
</script>
</head>
<%--   **********    --%>
<body>
<form id="frm_add_traderOffer" cssClass="formular">
		...
		<tr><td>
			...
			<table width="100">
				<tbody>
					<tr><s:select list="listGrade" label="Grade *" name="gradeOfferId"  value="raGradeMaster" headerKey="0" headerValue="--select--"  listKey="gradeId" listValue="gradeDispName"></s:select></tr>
					<tr><td><s:select list="listOrigin" label="Origin *" name="OriginOfferId"  value="raOriginMaster" headerKey="0" headerValue="--select--"  listKey="originId" listValue="originDispName"></s:select></td></tr>
					...
					<tr>
						...
						<td>Load Port</td><td><s:select name="loadPortNameId" list="listLoadPort" label="Load_Port*"listKey="loadId" listValue="loadPortType" value="raLoadPort"
				...
				<tr>
					...
					<td>Destination Port</td><td>	<s:select list="listDestPort" name="destPortOfferId" label="Destination Port*"
					...
					<tr>
						...
						<td>Product Name</td><td><s:select list="listProduct1" name="productId" label="Product Name*"
							value="raProductMaster" headerKey="0" headerValue="--Select--"
							listKey="productId" listValue="description" theme="simple"></s:select>
					</td>
					</tr>
					<tr>
					<td>Comments</td><td><s:textarea rows="5" cols="5" name="raTraceOffers.comments" theme="simple" ></s:textarea></td></tr>
		
					<tr>
					
					<td>	Farm name:</td><td><s:select list="listFarm" name="farmId" label="Farm Name "
		...
		<tr></tr>
					...
					<s:if test="raTraceOffers.offerId!=null">
						<s:submit value="Update" action="updateTraderOffers" theme="simple" id="button_style"></s:submit>
					</s:if>
					<s:else>
						<s:submit value="Save" action="addTraderOffers" theme="simple" id="button_style"></s:submit>
					</s:else>
					
					<s:submit value="Reset" action="addTraderOffers" theme="simple" id="button_style"></s:submit>
